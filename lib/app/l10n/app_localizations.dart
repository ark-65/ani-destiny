...
class AppLocalizations {
  ...
  String clearEndedDownloadsRetainedDiscardedClearActionNote(
    ...
    return switch (_languageCode) {
      ...
      _ =>
        '标记为“待清理残留文件”的任务会继续留在列表里，直到这份半截文件已经被手动删掉，或 AniDestiny 成功把它清掉。上面的“$actionLabel”可以先清掉其他已经收尾的任务；删完这份残留后，再回来点这条任务的“重新检查”。',
    };
  }
  ...
  String clearEndedDownloadsRetainedDiscardedRemoveActionNote(
    ...
    return switch (_languageCode) {
      ...
      _ =>
        '标记为“待清理残留文件”的任务会继续留在列表里，直到这份半截文件已经被手动删掉，或 AniDestiny 成功把它清掉。已经可以收尾的那条任务，现在可以直接点“$actionLabel”；删完这份残留后，再回来点这条任务的“重新检查”。',
    };
  }
  ...
  String clearEndedDownloadsRetainedDiscardedBatchRecheckNote(
    ...
    return switch (_languageCode) {
      ...
      _ =>
        '标记为“待清理残留文件”的任务会继续留在列表里，直到这些半截文件已经被手动删掉，或 AniDestiny 成功把它们清掉。删完后可以先点上面的“$actionLabel”，也可以逐条点“重新检查”。',
    };
  }
  ...
  String clearEndedDownloadsRetainedDiscardedBatchClearActionNote(
    ...
    return switch (_languageCode) {
      ...
      _ =>
        '标记为“待清理残留文件”的任务会继续留在列表里，直到这些半截文件已经被手动删掉，或 AniDestiny 成功把它们清掉。上面的“$clearActionLabel”可以先清掉其他已经收尾的任务；删完这些残留后，再点上面的“$recheckActionLabel”，或逐条点“重新检查”。',
    };
  }
  ...
  String clearEndedDownloadsRetainedDiscardedBatchRemoveActionNote(
    ...
    return switch (_languageCode) {
      ...
      _ =>
        '标记为“待清理残留文件”的任务会继续留在列表里，直到这些半截文件已经被手动删掉，或 AniDestiny 成功把它们清掉。已经可以收尾的那条任务，现在可以直接点“$removeActionLabel”；删完这些残留后，再点上面的“$recheckActionLabel”，或逐条点“重新检查”。',
    };
  }
...
const _localizedValues = {
  'zh': {
    ...
    'clearEndedDownloadsManualCleanupRemaining':
        '标记为“待清理残留文件”的任务会继续留在列表里，直到这些半截文件已经被手动删掉，或 AniDestiny 成功把它们清掉。',
    'mock': 'Mock',
    ...
    'sourceUnavailableSuggestion': '上游数据源可能已变化或暂时不可用。请先切换到其他数据源后重试。先返回该集源列表，回到该集源列表后切换到不同数据源并重试。若仍无效，再回到该集重新选择其他来源再试一次。',
  ...
  'en': {
    ...
    'sourceUnavailableSuggestion':
        'The source may have changed or is temporarily unavailable. Switch to another source before retrying. Return to this episode\u2019s source list, then select a different source and retry. If it still fails, return to the current item and choose a different source again before retrying.',
  ...
  'ja': {
    ...
    'sourceUnavailableSuggestion':
        '上流ソースが変更されたか、一時的に利用できない状態になっています。まずこのエピソードのソース一覧に戻って、まず別のソースへ切り替えてから再試行してください。必要なら現在の画面でソースを再選択してから再度再試行してください。',
